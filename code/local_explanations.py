...
bamultishapes_classes_names = ["house", "grid", "wheel",
                               "ba", "house+grid", "house+wheel", "wheel+grid", "all"]
...
def elbow_method(weights, index_stopped=None, min_num_include=7, backup=None):
    ...
    sorted_weights = np.convolve(sorted_weights, np.ones(
        min_num_include), 'valid') / min_num_include
...
def label_explanations(adjs, num_graphs):
    classes = []
    classes_names = ["house", "grid", "wheel", "ba",
                     "house+grid", "house+wheel", "wheel+grid", "all"]
    for k in range(num_graphs):
        G = nx.from_numpy_array(adjs[k])
...
def read_bamultishapes(explainer="PGExplainer", dataset="BAMultiShapes", model="GCN", split="TRAIN", evaluate_method=False, remove_mix=False, min_num_include=5, manual_cut=None):
    ...
    # (total_graph_labels, total_cc_labels)
    ...
    for split in [split]:
        ...
        for c in ["0", "1"]:
            for pp in os.listdir(path + c + "/"):
                ...
                if manual_cut is None:
                    cut = elbow_method(np.triu(adj).flatten(
                    ), index_stopped, min_num_include=min_num_include)
                else:
                    cut = manual_cut
                ...
                for cc in connected_components:
                    if len(cc) >= 2 and ego_node in cc:  # to exclude single nodes
                        ...
                        if priori_annotation:
                            ...
                            names = "".join(
                                sorted(convert_labels_to_names(no_ego_attrs)))
